...
load_env_vars() {
  ...
  ENV_VARS[DISCOVERY_QUERY_LIMIT]="50"
  ENV_VARS[DISCOVERY_RESULT_LIMIT]="10"
  ENV_VARS[AI_ANALYSIS_LIMIT]="60"
  ENV_VARS[MAX_VERIFICATIONS_PER_RUN]="25"
  ENV_VARS[MAX_SOURCE_FETCHES_PER_RUN]="60"
  ENV_VARS[NVIDIA_BASE_URL]="https://integrate.api.nvidia.com/v1"
  ENV_VARS[NVIDIA_MODEL]="nvidia/nemotron-3-super-120b-a12b"

  # Monitor worker (M19.5). Region is required: observations without one can
  # never confirm a price event.
  ENV_VARS[MONITOR_ENABLED]="true"
  ENV_VARS[MONITOR_OBSERVED_REGION]="US"
  ENV_VARS[MONITOR_DAILY_FETCH_BUDGET]="50"
  ENV_VARS[MONITOR_CONCURRENCY]="2"
  ENV_VARS[MONITOR_PER_DOMAIN_RPM]="20"

  # Alerts stay in dry-run until deliverability is verified.
  ENV_VARS[EMAIL_DRY_RUN]="true"
  ENV_VARS[EMAIL_DAILY_BUDGET]="500"

  # M19 feature flags: all OFF until their STOP gates pass.
  ENV_VARS[FEATURE_TOPIC_PAGES]="false"
  ENV_VARS[FEATURE_TRACKER_UI]="false"
  ENV_VARS[FEATURE_PRICE_ALERTS]="false"
  ENV_VARS[FEATURE_AUTO_STATUS]="false"
...
EOF
  ...
  for key in DATABASE_URL APP_URL AUTH_SECRET CRON_SECRET ADMIN_EMAILS ADMIN_BOOTSTRAP_PASSWORD \
    ...
    MAX_SOURCE_FETCHES_PER_RUN MAX_VERIFICATIONS_PER_RUN \
    MAX_VERIFICATIONS_PER_RUN NVIDIA_BASE_URL NVIDIA_MODEL TAVILY_API_KEY NVIDIA_API_KEY \
    MONITOR_ENABLED MONITOR_OBSERVED_REGION MONITOR_DAILY_FETCH_BUDGET MONITOR_CONCURRENCY \
    MONITOR_PER_DOMAIN_RPM EMAIL_DRY_RUN EMAIL_DAILY_BUDGET \
    FEATURE_TOPIC_PAGES FEATURE_TRACKER_UI FEATURE_PRICE_ALERTS FEATURE_AUTO_STATUS; do
